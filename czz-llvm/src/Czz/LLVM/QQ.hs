...
import qualified Lang.Crucible.LLVM.Intrinsics as IC
...
parseOvrType :: AT.Parser (QQType, [QQType], Bool)
parseOvrType = do
  ret <- parseType'
  void $ AT.char '@'
  (args, varargs) <- parseArgList
  AT.skipSpace
  return (ret, args, varargs)
...
liftToCrucibleType :: QQType -> Q Type
liftToCrucibleType tp =
  case tp of
    QQVar {}      -> error "liftQQCrucibleType: can't handle vars"
    QQIntVar nm   -> [t| BVType $(varT (mkName nm)) |]
    QQSizeT       -> [t| BVType $(varT (mkName "wptr")) |]
    QQSSizeT      -> [t| BVType $(varT (mkName "wptr")) |]
    QQPrim pt     -> liftPrim pt
    QQPtrTo _t    -> [t| LLVMPointerType $(varT (mkName "wptr")) |]
    QQArray _ t'  -> [t| VectorType $(liftToCrucibleType t') |]
    QQVector _ t' -> [t| VectorType $(liftToCrucibleType t') |]
    QQStruct ts   -> [t| StructType $(liftTypeArgs ts False) |]
    QQPackedStruct ts -> [t| StructType $(liftTypeArgs ts False) |]
    QQAlias{} -> fail $ "Cannot lift alias type " ++ show tp
    QQOpaque  -> fail $ "Cannot lift opaque type " ++ show tp
    QQFunTy{} -> fail $ "Cannot lift function type " ++ show tp
 where
  liftPrim pt = case pt of
    L.Void         -> [t| UnitType |]
    L.Integer n    -> [t| BVType $(litT (return (NumTyLit (fromIntegral n)))) |]
    L.FloatType ft -> [t| FloatType $(liftFloatType ft) |]
    L.Label    -> fail $ "Cannot lift label type " ++ show pt
    L.X86mmx   -> fail $ "Cannot lift X86mmx type " ++ show pt
    L.Metadata -> fail $ "Cannot lift metatata type " ++ show pt

  liftFloatType ft = case ft of
    L.Half      -> [t| HalfFloat |]
    L.Float     -> [t| SingleFloat |]
    L.Double    -> [t| DoubleFloat |]
    L.Fp128     -> [t| QuadFloat |]
    L.X86_fp80  -> [t| X86_80Float |]
    L.PPC_fp128 -> [t| DoubleDoubleFloat |]

liftTypeArgs :: [QQType] -> Bool -> Q Type
liftTypeArgs = go [t| Ctx.EmptyCtx |]
 where
 go :: Q Type -> [QQType] -> Bool -> Q Type
 go xs [] True  = [t| $(xs) Ctx.::> VectorType AnyType |]
 go xs [] False = xs
 go xs (t:ts) varargs = go [t| $(xs) Ctx.::> $(liftToCrucibleType t) |] ts varargs

liftOvrType :: QQType -> [QQType] -> Bool -> Q Type
liftOvrType ret args varargs =
  [t| IC.LLVMOverride
        $(varT (mkName "p"))
        $(varT (mkName "sym"))
        $(liftTypeArgs args varargs)
        $(liftToCrucibleType ret) |]
...
llvmCrucibleType :: QuasiQuoter
llvmCrucibleType =
  QuasiQuoter
  { quoteExp = error "llvmCrucibleType cannot quasiquote an expression"
  , quotePat = error "llvmCrucibleType cannot quasiquote a pattern"
  , quoteType = \str ->
       do case AT.parseOnly parseType' (T.pack str) of
            Left msg -> error msg
            Right x  -> liftToCrucibleType x
  , quoteDec = error "llvmCrucibleType cannot quasiquote a declaration"
  }

llvmArgs :: QuasiQuoter
llvmArgs =
  QuasiQuoter
  { quoteExp = error "llvmArgs cannot quasiquote an expression"
  , quotePat = error "llvmArgs cannot quasiquote a pattern"
  , quoteType = \str ->
       do case AT.parseOnly parseArgList' (T.pack str) of
            Left msg -> error msg
            Right (args, varargs)  -> liftTypeArgs args varargs
  , quoteDec = error "llvmArgs cannot quasiquote a declaration"
  }
...
llvmOvrType :: QuasiQuoter
llvmOvrType =
  QuasiQuoter
  { quoteExp = error "llvmOvrType cannot quasiquote an expression"
  , quotePat = error "llvmOvrType cannot quasiquote a pattern"
  , quoteType = \str ->
       do case AT.parseOnly parseOvrType (T.pack str) of
            Left msg -> error msg
            Right (ret, args, varargs) -> liftOvrType ret args varargs
  , quoteDec = error "llvmOvrType cannot quasiquote a declaration"
  }
